...
int mix_stats_record(mix_stats_t *st, int active_count, int muted_count, uint64_t latency_us) {
    ...
    if (latency_us > 0) {
        ...
        if (lat < st->min_latency_us)
            st->min_latency_us = lat;
        if (lat > st->max_latency_us)
            st->max_latency_us = lat;
    }
    return 0;
}
...
int mix_stats_snapshot(const mix_stats_t *st, mix_stats_snapshot_t *out) {
    ...
    out->avg_latency_us = (st->mix_calls > 0) ? (st->latency_sum_us / (double)st->mix_calls) : 0.0;
