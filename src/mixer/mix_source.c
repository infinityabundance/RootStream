...
int mix_source_init(mix_source_t  *src,
    ...
    if (weight > MIX_WEIGHT_MAX) weight = MIX_WEIGHT_MAX;
    src->weight = weight;
    if (name)
...
const char *mix_src_type_name(mix_src_type_t t) {
    switch (t) {
    case MIX_SRC_CAPTURE:    return "CAPTURE";
    case MIX_SRC_MICROPHONE: return "MICROPHONE";
    case MIX_SRC_LOOPBACK:   return "LOOPBACK";
    case MIX_SRC_SYNTH:      return "SYNTH";
