...
void mix_engine_destroy(mix_engine_t *e) { free(e); }

int mix_engine_source_count(const mix_engine_t *e) { return e ? e->count : 0; }
...
int mix_engine_add_source(mix_engine_t *e, const mix_source_t *src) {
    if (!e || !src) return -1;
    if (e->count >= MIX_MAX_SOURCES) return -1;
    if (find_slot(e, src->id) >= 0) return -1; /* duplicate */
...
int mix_engine_mix(mix_engine_t        *e,
                   ...
                   int16_t              *out,
                   int                   frames) {
    if (!e || !inputs || !src_ids || !out || frames <= 0) return -1;
    if (frames > MIX_MAX_FRAMES) return -1;

    mix_engine_silence(out, frames);

    for (int s = 0; s < src_count; s++) {
        if (!inputs[s]) continue;
        int slot = find_slot(e, src_ids[s]);
        if (slot < 0) continue;
        const mix_source_t *src = &e->sources[slot];
        if (src->muted || src->weight == 0.0f) continue;
