...
relay_client_t *relay_client_create(const relay_io_t *io,
                                     const uint8_t    *token,
                                     bool              is_host) {
    if (!io || !io->send_fn || !token) return NULL;
...
relay_session_id_t relay_client_get_session_id(const relay_client_t *client) {
    return (client && client->state == RELAY_CLIENT_READY)
           ? client->session_id : 0;
}

int relay_client_connect(relay_client_t *client) {
    if (!client) return -1;
    if (client->state != RELAY_CLIENT_DISCONNECTED) return -1;
...
int relay_client_receive(relay_client_t *client,
                          ...
                          void           *data_ud) {
    if (!client || !buf || len < (size_t)RELAY_HDR_SIZE) return -1;

    relay_header_t hdr;
    if (relay_decode_header(buf, &hdr) != 0) return -1;
...
int relay_client_send_data(relay_client_t *client,
    ...
    if (client->state != RELAY_CLIENT_READY) return -1;
    if (payload_len > RELAY_MAX_PAYLOAD) return -1;
