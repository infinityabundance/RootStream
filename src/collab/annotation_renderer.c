...
static stroke_t *find_stroke(annotation_renderer_t *r, uint32_t stroke_id) {
    for (int i = 0; i < r->stroke_count; i++) {
        if (r->strokes[i].stroke_id == stroke_id)
            return &r->strokes[i];
    }
    return NULL;
}
...
void annotation_renderer_apply_event(annotation_renderer_t *renderer,
    ...
    switch (event->type) {
        case ANNOT_DRAW_BEGIN:
            if (renderer->stroke_count >= ANNOT_RENDERER_MAX_STROKES)
                return;
            {
        ...
        case ANNOT_DRAW_POINT: {
            stroke_t *s = find_stroke(renderer, event->draw_point.stroke_id);
...
static void draw_circle(uint8_t *pixels, int width, int height, int stride, int cx, int cy, int r,
                        annot_color_t color) {
...
static void draw_line(uint8_t *pixels, int width, int height, int stride, int x0, int y0, int x1,
                      int y1, int r, annot_color_t color) {
    int dx = abs(x1 - x0);
    int dy = abs(y1 - y0);
    int sx = (x0 < x1) ? 1 : -1;
    int sy = (y0 < y1) ? 1 : -1;
    ...
    for (int i = 0; i < renderer->stroke_count; i++) {
        ...
        for (int j = 0; j < s->count; j++) {
            ...
            if (j == 0) {
                ...
                draw_line(pixels, width, height, stride, px0, py0, px, py, r, s->color);
            }
        }
    }
