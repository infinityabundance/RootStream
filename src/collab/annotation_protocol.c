...
int annotation_encode(const annotation_event_t *event, uint8_t *buf, size_t buf_sz) {
    ...
    switch (event->type) {
        case ANNOT_DRAW_BEGIN:
            write_f32(p, event->draw_begin.pos.x);
            p += 4;
            write_f32(p, event->draw_begin.pos.y);
            p += 4;
            write_u32_le(p, event->draw_begin.color);
            p += 4;
            write_f32(p, event->draw_begin.width);
            p += 4;
            write_u32_le(p, event->draw_begin.stroke_id);
            break;
        case ANNOT_DRAW_POINT:
            write_f32(p, event->draw_point.pos.x);
            p += 4;
            write_f32(p, event->draw_point.pos.y);
            p += 4;
        ...
        case ANNOT_DRAW_END:
            ...
            break;
        case ANNOT_ERASE:
            write_f32(p, event->erase.center.x);
            p += 4;
            write_f32(p, event->erase.center.y);
            p += 4;
        ...
        case ANNOT_CLEAR_ALL:
            break;
        case ANNOT_TEXT: {
            write_f32(p, event->text.pos.x);
            p += 4;
            write_f32(p, event->text.pos.y);
            p += 4;
            write_u32_le(p, event->text.color);
            p += 4;
            write_f32(p, event->text.font_size);
            p += 4;
        ...
        case ANNOT_POINTER_MOVE:
            write_f32(p, event->pointer_move.pos.x);
            p += 4;
            write_f32(p, event->pointer_move.pos.y);
            p += 4;
...
int annotation_decode(const uint8_t *buf, size_t buf_sz, annotation_event_t *event) {
    if (!buf || !event || buf_sz < ANNOTATION_HDR_SIZE)
        return -1;

    uint16_t magic = read_u16_le(buf);
    if (magic != (uint16_t)ANNOTATION_MAGIC)
        return -1;
    if (buf[2] != ANNOTATION_VERSION)
        return -1;
    ...
    switch (event->type) {
        case ANNOT_DRAW_BEGIN:
            if (remaining < 20)
                return -1;
            event->draw_begin.pos.x = read_f32(p);
            p += 4;
            event->draw_begin.pos.y = read_f32(p);
            p += 4;
            event->draw_begin.color = read_u32_le(p);
            p += 4;
            event->draw_begin.width = read_f32(p);
            p += 4;
        ...
        case ANNOT_DRAW_POINT:
            if (remaining < 12)
                return -1;
            event->draw_point.pos.x = read_f32(p);
            p += 4;
            event->draw_point.pos.y = read_f32(p);
            p += 4;
        ...
        case ANNOT_ERASE:
            if (remaining < 12)
                return -1;
            event->erase.center.x = read_f32(p);
            p += 4;
            event->erase.center.y = read_f32(p);
            p += 4;
        ...
        case ANNOT_TEXT:
            if (remaining < 18)
                return -1;
            event->text.pos.x = read_f32(p);
            p += 4;
            event->text.pos.y = read_f32(p);
            p += 4;
            event->text.color = read_u32_le(p);
            p += 4;
            event->text.font_size = read_f32(p);
            p += 4;
            event->text.text_len = read_u16_le(p);
            p += 2;
            if (event->text.text_len > ANNOTATION_MAX_TEXT)
                return -1;
            if (remaining - 18 < event->text.text_len)
                return -1;
        ...
        case ANNOT_POINTER_MOVE:
            if (remaining < 12)
                return -1;
            event->pointer_move.pos.x = read_f32(p);
            p += 4;
            event->pointer_move.pos.y = read_f32(p);
            p += 4;
