...
int analytics_event_decode(const uint8_t     *buf,
    ...
    if (!buf || !event || buf_sz < ANALYTICS_HDR_SIZE) return -1;
    if (r32le(buf) != (uint32_t)ANALYTICS_MAGIC) return -1;
    ...
    if (event->payload_len > ANALYTICS_MAX_PAYLOAD) return -1;
    if (buf_sz < ANALYTICS_HDR_SIZE + (size_t)event->payload_len) return -1;
    if (event->payload_len > 0)
...
const char *analytics_event_type_name(analytics_event_type_t type) {
    switch (type) {
    case ANALYTICS_VIEWER_JOIN:    return "viewer_join";
    case ANALYTICS_VIEWER_LEAVE:   return "viewer_leave";
    case ANALYTICS_BITRATE_CHANGE: return "bitrate_change";
    case ANALYTICS_FRAME_DROP:     return "frame_drop";
    case ANALYTICS_QUALITY_ALERT:  return "quality_alert";
    case ANALYTICS_SCENE_CHANGE:   return "scene_change";
    case ANALYTICS_STREAM_START:   return "stream_start";
    case ANALYTICS_STREAM_STOP:    return "stream_stop";
    case ANALYTICS_LATENCY_SAMPLE: return "latency_sample";
