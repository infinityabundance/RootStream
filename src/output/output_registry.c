...
int output_registry_active_count(const output_registry_t *r) {
    ...
    for (int i = 0; i < OUTPUT_MAX_TARGETS; i++)
        if (r->used[i] && r->targets[i].state == OT_ACTIVE)
            n++;
    return n;
}
...
static int find_slot(const output_registry_t *r, const char *name) {
    for (int i = 0; i < OUTPUT_MAX_TARGETS; i++)
        if (r->used[i] && strncmp(r->targets[i].name, name, OUTPUT_NAME_MAX) == 0)
            return i;
    return -1;
}
...
output_target_t *output_registry_add(output_registry_t *r, const char *name, const char *url,
    ...
    if (!r || !name)
        return NULL;
    if (r->count >= OUTPUT_MAX_TARGETS)
        return NULL;
    if (find_slot(r, name) >= 0)
