...
int metadata_export_json(const stream_metadata_t *meta, char *buf, size_t buf_sz) {
    ...
    int n = snprintf(buf, buf_sz,
                     ...
                     "\"tags\":\"%s\""
                     "}",
                     meta->start_us, meta->duration_us, meta->video_width, meta->video_height,
                     (unsigned)meta->video_fps, (unsigned)meta->flags,
                     (meta->flags & METADATA_FLAG_LIVE) ? "true" : "false", meta->title,
...
static int kv_json_cb(const char *key, const char *value, void *ud) {
    ...
    int r = snprintf(ctx->buf + ctx->pos, ctx->buf_sz - ctx->pos, "%s\"%s\":\"%s\"",
                     ctx->first ? "" : ",", key, value);
