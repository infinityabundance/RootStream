...
class EventStore {
   public:
     ...
     * @return 0 on success, negative on error
     */
    int getEvents(const std::string& aggregateType, uint32_t aggregateId,
                  std::vector<Event>& events, uint32_t fromVersion = 0);
     ...
     * @return 0 on success, negative on error
     */
    int createSnapshot(const std::string& aggregateType, uint32_t aggregateId, uint32_t version,
     ...
     * @return 0 on success, negative on error
     */
    int getSnapshot(const std::string& aggregateType, uint32_t aggregateId, nlohmann::json& state,
                    uint32_t& version);
     ...
     * @return 0 on success, negative on error
     */
    int getAuditTrail(uint32_t userId, std::vector<Event>& events, uint64_t fromTime = 0);
