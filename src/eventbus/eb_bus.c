...
eb_bus_t *eb_bus_create(void) {
    ...
    if (b) b->next_handle = 0;
    return b;
}

void eb_bus_destroy(eb_bus_t *b) { free(b); }

int eb_bus_subscriber_count(const eb_bus_t *b) { return b ? b->count : 0; }

eb_handle_t eb_bus_subscribe(eb_bus_t     *b,
                              eb_type_t     type_id,
                              eb_callback_t cb,
                              void         *user) {
    if (!b || !cb || b->count >= EB_MAX_SUBSCRIBERS) return EB_INVALID_HANDLE;
...
int eb_bus_publish(eb_bus_t *b, const eb_event_t *e) {
    ...
    int dispatched = 0;
    for (int i = 0; i < EB_MAX_SUBSCRIBERS; i++) {
        if (!b->subs[i].in_use) continue;
        if (b->subs[i].type_id == EB_TYPE_ANY ||
            b->subs[i].type_id == e->type_id) {
