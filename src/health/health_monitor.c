...
health_metric_t *health_monitor_register(health_monitor_t *hm, const char *name, hm_kind_t kind) {
    if (!hm || !name || hm->count >= HEALTH_MAX_METRICS)
        return NULL;
    /* Reject duplicates */
    for (int i = 0; i < HEALTH_MAX_METRICS; i++) {
        if (hm->used[i] && strncmp(hm->metrics[i].name, name, HEALTH_METRIC_NAME_MAX) == 0)
            return NULL;
    }
    for (int i = 0; i < HEALTH_MAX_METRICS; i++) {
        if (!hm->used[i]) {
...
int health_monitor_evaluate(health_monitor_t *hm, health_summary_t *out) {
    ...
    for (int i = 0; i < HEALTH_MAX_METRICS; i++) {
        ...
        if (lv == HM_CRIT) {
            out->n_crit++;
            if (out->overall < HM_CRIT)
                out->overall = HM_CRIT;
        } else if (lv == HM_WARN) {
            out->n_warn++;
            if (out->overall < HM_WARN)
                out->overall = HM_WARN;
        } else {
            out->n_ok++;
        }
