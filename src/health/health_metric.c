...
int hm_init(health_metric_t *m, const char *name, hm_kind_t kind) {
    ...
    m->kind = kind;
    if (name)
        strncpy(m->name, name, HEALTH_METRIC_NAME_MAX - 1);
    return 0;
}
...
hm_level_t hm_evaluate(const health_metric_t *m) {
    ...
    if (val <= m->thresh.crit_lo || val >= m->thresh.crit_hi)
        return HM_CRIT;
    if (val <= m->thresh.warn_lo || val >= m->thresh.warn_hi)
        return HM_WARN;
    return HM_OK;
}
