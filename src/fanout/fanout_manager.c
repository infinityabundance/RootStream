 ...
 */
static int send_frame(int fd, const uint8_t *data, size_t size,
    ...
    if (wr != (ssize_t)sizeof(hdr)) return -1;

    wr = send(fd, data, size, MSG_NOSIGNAL);
    if (wr != (ssize_t)size) return -1;
...
static void deliver_to_session(const session_entry_t *entry,
                                void *user_data) {
...
int fanout_manager_deliver(fanout_manager_t   *mgr,
    ...
    mgr->stats.frames_in++;
    if (ctx.delivered > 0) mgr->stats.frames_delivered++;
