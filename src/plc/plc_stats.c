...
static void window_push(plc_stats_t *st, int is_lost) {
    if (st->win_count >= PLC_STATS_WINDOW) {
        /* Evict oldest */
        int oldest = (st->win_head - st->win_count + PLC_STATS_WINDOW * 2) % PLC_STATS_WINDOW;
        if (st->window[oldest])
            st->win_lost_count--;
        st->win_count--;
    }
...
int plc_stats_snapshot(const plc_stats_t *st, plc_stats_snapshot_t *out) {
    ...
    out->loss_rate = (st->win_count > 0) ? (double)st->win_lost_count / (double)st->win_count : 0.0;
    return 0;
}
