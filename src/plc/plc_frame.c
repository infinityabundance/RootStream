...
int plc_frame_byte_size(const plc_frame_t *frame) {
    if (!frame)
        return -1;
    return (int)(PLC_FRAME_HDR_SIZE + (size_t)frame->channels * (size_t)frame->num_samples * 2);
}

int plc_frame_encode(const plc_frame_t *frame, uint8_t *buf, size_t buf_sz) {
    if (!frame || !buf)
        return -1;
    if (frame->channels == 0 || frame->channels > PLC_MAX_CHANNELS)
        return -1;
    if (frame->num_samples == 0 || frame->num_samples > PLC_MAX_SAMPLES_PER_CH)
        return -1;

    int sz = plc_frame_byte_size(frame);
    if (sz < 0 || buf_sz < (size_t)sz)
        return -1;
...
int plc_frame_decode(const uint8_t *buf, size_t buf_sz, plc_frame_t *frame) {
    if (!buf || !frame || buf_sz < PLC_FRAME_HDR_SIZE)
        return -1;
    if (r32le(buf) != (uint32_t)PLC_FRAME_MAGIC)
        return -1;
    ...
    size_t n_samples = (size_t)frame->channels * (size_t)frame->num_samples;
    if (buf_sz < PLC_FRAME_HDR_SIZE + n_samples * 2)
        return -1;
