...
gop_controller_t *gop_controller_create(const gop_policy_t *policy) {
    if (!policy || gop_policy_validate(policy) != 0) return NULL;
...
void gop_controller_destroy(gop_controller_t *gc) { free(gc); }

int gop_controller_update_policy(gop_controller_t  *gc,
                                   const gop_policy_t *policy) {
    if (!gc || !policy || gop_policy_validate(policy) != 0) return -1;
    gc->policy = *policy;
    return 0;
}
...
gop_decision_t gop_controller_next_frame(gop_controller_t *gc,
    ...
    if (!gc) {
        if (reason_out) *reason_out = GOP_REASON_NONE;
    ...
    if (scene_score >= p->scene_change_threshold) {
        gc->frames_since_idr = 0;
        if (reason_out) *reason_out = GOP_REASON_SCENE_CHANGE;
        return GOP_DECISION_IDR;
    }
    ...
    if (loss >= p->loss_threshold && rtt_us < p->rtt_threshold_us) {
        gc->frames_since_idr = 0;
        if (reason_out) *reason_out = GOP_REASON_LOSS_RECOVERY;
        return GOP_DECISION_IDR;
    }
...
const char *gop_decision_name(gop_decision_t d) {
    switch (d) {
    case GOP_DECISION_P_FRAME: return "P_FRAME";
    case GOP_DECISION_IDR:     return "IDR";
...
const char *gop_reason_name(gop_reason_t r) {
    switch (r) {
    case GOP_REASON_NATURAL:       return "NATURAL";
    case GOP_REASON_SCENE_CHANGE:  return "SCENE_CHANGE";
    case GOP_REASON_LOSS_RECOVERY: return "LOSS_RECOVERY";
    case GOP_REASON_NONE:          return "NONE";
    default:                       return "UNKNOWN";
    }
}
