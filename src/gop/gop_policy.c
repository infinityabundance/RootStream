...
int gop_policy_validate(const gop_policy_t *p) {
    if (!p) return -1;
    if (p->min_gop_frames < 1)  return -1;
    if (p->max_gop_frames < p->min_gop_frames) return -1;
    if (p->scene_change_threshold < 0.0f || p->scene_change_threshold > 1.0f) return -1;
    if (p->loss_threshold < 0.0f || p->loss_threshold > 1.0f) return -1;
    return 0;
}
