...
class RecordingManager {
   ...
   public:
    ...
    // Frame submission
    int submit_video_frame(const uint8_t *frame_data, uint32_t width, uint32_t height,
                           const char *pixel_format, uint64_t timestamp_us);

    int submit_audio_chunk(const float *samples, uint32_t sample_count, uint32_t sample_rate,
   ...
   private:
    ...
    int init_video_encoder(enum VideoCodec codec, uint32_t width, uint32_t height, uint32_t fps,
                           uint32_t bitrate_kbps);
    int init_muxer(enum ContainerFormat format);
    int encode_frame_with_active_encoder(const uint8_t *frame_data, uint32_t width, uint32_t height,
