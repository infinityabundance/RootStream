...
static void entry_group_callback(AvahiEntryGroup *g, AvahiEntryGroupState state, void *userdata) {
    ...
    switch (state) {
        ...
        case AVAHI_ENTRY_GROUP_COLLISION: {
            /* A service with the same name already exists on the network.
             * Generate an alternative name using avahi_alternative_service_name()
             * and re-register so that both instances coexist without conflict. */
            const char *current_name = avahi->ctx ? avahi->ctx->keypair.identity : "rootstream";
            char *alt_name = avahi_alternative_service_name(current_name);
            if (alt_name) {
                fprintf(stderr, "WARNING: Service name collision — retrying as '%s'\n", alt_name);
                avahi_free(alt_name);
            } else {
                fprintf(stderr, "WARNING: Service name collision and rename failed\n");
            }
            break;
        }
    ...
    if (event == AVAHI_RESOLVER_FOUND) {
        ...
        if (capability_txt) {
            ...
            if (avahi_string_list_get_pair(capability_txt, &key, &value, &value_len) >= 0) {
                strncpy(cache_entry.capability, value, sizeof(cache_entry.capability) - 1);
                avahi_free(key);
                avahi_free(value);
            }
        } else {
        ...
        if (version_txt) {
            ...
            if (avahi_string_list_get_pair(version_txt, &key, &value, &value_len) >= 0) {
                strncpy(cache_entry.version, value, sizeof(cache_entry.version) - 1);
                avahi_free(key);
                avahi_free(value);
            }
        }
        ...
        if (bandwidth_txt) {
            ...
            if (avahi_string_list_get_pair(bandwidth_txt, &key, &value, &value_len) >= 0) {
                strncpy(cache_entry.bandwidth, value, sizeof(cache_entry.bandwidth) - 1);
                avahi_free(key);
                avahi_free(value);
            }
        }
        ...
        if (strlen(cache_entry.rootstream_code) > 0 && ctx->num_peers < MAX_PEERS) {
            ...
            if (avahi_address_parse(addr_str, AVAHI_PROTO_INET, (AvahiAddress *)&addr->sin_addr) !=
                NULL) {
                /* Store peer info */
                ...
                printf("  → Added peer: %s (code: %.8s..., %s)\n", peer->hostname,
                       peer->rootstream_code, cache_entry.capability);
...
static void browse_callback(AvahiServiceBrowser *b, AvahiIfIndex interface, AvahiProtocol protocol,
    ...
    switch (event) {
        case AVAHI_BROWSER_NEW:
            ...
            if (!avahi_service_resolver_new(avahi->client, interface, protocol, name, type, domain,
                                            AVAHI_PROTO_UNSPEC, 0, resolve_callback, avahi)) {
...
int discovery_init(rootstream_ctx_t *ctx) {
    ...
    /* Create client */
    int error;
    avahi->client = avahi_client_new(avahi_simple_poll_get(avahi->simple_poll), 0, /* flags */
...
int discovery_announce(rootstream_ctx_t *ctx) {
    ...
    if (avahi && avahi->client) {
        ...
        if (!avahi->group) {
            avahi->group = avahi_entry_group_new(avahi->client, entry_group_callback, avahi);
            if (!avahi->group) {
        ...
        snprintf(pubkey_txt, sizeof(pubkey_txt), "code=%s", ctx->keypair.rootstream_code);
        ...
        /* Add service */
        int ret =
            avahi_entry_group_add_service_strlst(avahi->group, AVAHI_IF_UNSPEC, /* All interfaces */
...
int discovery_browse(rootstream_ctx_t *ctx) {
    ...
    if (avahi && avahi->client) {
        ...
        /* Create browser */
        avahi->browser =
            avahi_service_browser_new(avahi->client, AVAHI_IF_UNSPEC, /* All interfaces */
                                      AVAHI_PROTO_UNSPEC,             /* IPv4 and IPv6 */
                                      "_rootstream._udp",             /* Service type */
                                      NULL,                           /* Domain (use default) */
                                      0,                              /* flags */
                                      browse_callback, avahi);
...
void discovery_print_stats(rootstream_ctx_t *ctx) {
    ...
    if (ctx->discovery.num_cached_peers > 0) {
        ...
        for (int i = 0; i < ctx->discovery.num_cached_peers; i++) {
            peer_cache_entry_t *entry = &ctx->discovery.peer_cache[i];
            printf("  %d. %s (%s:%u) - %s %s\n", i + 1, entry->hostname, entry->ip_address,
                   entry->port, entry->capability, entry->is_online ? "[online]" : "[offline]");
        }
    }
    printf("\n");
}
