...
checkpoint_manager_t *checkpoint_manager_create(const checkpoint_config_t *config) {
    ...
    if (config) {
        ...
        m->max_keep = (config->max_keep > 0) ? config->max_keep : CHECKPOINT_MAX_KEEP;
    } else {
...
static void ckpt_filename(char *out, size_t outsz, const char *dir, uint64_t session_id,
                          uint64_t seq) {
    snprintf(out, outsz, "%s/rootstream-ckpt-%llu-%llu.bin", dir, (unsigned long long)session_id,
...
int checkpoint_save(checkpoint_manager_t *mgr, const session_state_t *state) {
    ...
    snprintf(tmp_path, sizeof(tmp_path), "%s/.ckpt_tmp_%llu.bin", mgr->dir,
    ...
    ckpt_filename(final_path, sizeof(final_path), mgr->dir, state->session_id, mgr->seq++);
...
int checkpoint_load(const checkpoint_manager_t *mgr, uint64_t session_id, session_state_t *state) {
    ...
    char prefix[128];
    snprintf(prefix, sizeof(prefix), "rootstream-ckpt-%llu-", (unsigned long long)session_id);
