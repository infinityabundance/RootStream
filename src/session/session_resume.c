...
int resume_decode_request(const uint8_t *buf, size_t buf_sz, resume_request_t *req) {
    if (!buf || !req || buf_sz < RESUME_MSG_HDR_SIZE + REQUEST_PAYLOAD_SZ)
        return -1;
    if (r32le(buf) != (uint32_t)RESUME_TAG_REQUEST)
        return -1;
...
int resume_decode_accepted(const uint8_t *buf, size_t buf_sz, resume_accepted_t *acc) {
    if (!buf || !acc || buf_sz < RESUME_MSG_HDR_SIZE + ACCEPTED_PAYLOAD_SZ)
        return -1;
    if (r32le(buf) != (uint32_t)RESUME_TAG_ACCEPTED)
        return -1;
...
int resume_decode_rejected(const uint8_t *buf, size_t buf_sz, resume_rejected_t *rej) {
    if (!buf || !rej || buf_sz < RESUME_MSG_HDR_SIZE + REJECTED_PAYLOAD_SZ)
        return -1;
    if (r32le(buf) != (uint32_t)RESUME_TAG_REJECTED)
        return -1;
...
bool resume_server_evaluate(const resume_request_t *req, const session_state_t *server_state,
    ...
    if (memcmp(req->stream_key, server_state->stream_key, SESSION_STREAM_KEY_LEN) != 0) {
        if (out_rej) {
            out_rej->session_id = req->session_id;
