...
void kfr_handler_destroy(kfr_handler_t *h) { free(h); }
...
kfr_decision_t kfr_handler_submit(kfr_handler_t       *h,
                                    ...
                                    uint64_t             now_us) {
    if (!h || !msg) return KFR_DECISION_SUPPRESS;

    kfr_ssrc_entry_t *e = find_or_alloc(h, msg->ssrc);
    if (!e) return KFR_DECISION_SUPPRESS; /* registry full */

    /* Urgent requests always bypass the cooldown */
    if (msg->priority > 0 ||
        !e->has_forwarded ||
        (now_us - e->last_forward_us) >= h->cooldown_us) {
...
const char *kfr_decision_name(kfr_decision_t d) {
    switch (d) {
    case KFR_DECISION_FORWARD:  return "FORWARD";
    case KFR_DECISION_SUPPRESS: return "SUPPRESS";
