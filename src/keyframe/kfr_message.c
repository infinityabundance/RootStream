...
int kfr_message_encode(const kfr_message_t *msg,
                         uint8_t             *buf,
                         size_t               buf_sz) {
    if (!msg || !buf || buf_sz < KFR_MSG_SIZE) return -1;
    if (msg->type != KFR_TYPE_PLI && msg->type != KFR_TYPE_FIR) return -1;
...
int kfr_message_decode(const uint8_t *buf,
                         size_t         buf_sz,
                         kfr_message_t *msg) {
    if (!buf || !msg || buf_sz < KFR_MSG_SIZE) return -1;
    if (r32le(buf) != (uint32_t)KFR_MSG_MAGIC) return -1;
...
const char *kfr_type_name(kfr_type_t t) {
    switch (t) {
    case KFR_TYPE_PLI: return "PLI";
    case KFR_TYPE_FIR: return "FIR";
