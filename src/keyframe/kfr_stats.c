...
void kfr_stats_destroy(kfr_stats_t *st) { free(st); }
...
int kfr_stats_record(kfr_stats_t *st, int forwarded, int urgent) {
    ...
    if (forwarded) st->requests_forwarded++;
    else           st->requests_suppressed++;
    if (urgent)    st->urgent_requests++;
    return 0;
}
...
int kfr_stats_snapshot(const kfr_stats_t *st, kfr_stats_snapshot_t *out) {
    ...
    out->suppression_rate = (st->requests_received > 0) ?
        (double)st->requests_suppressed / (double)st->requests_received : 0.0;
    return 0;
}
