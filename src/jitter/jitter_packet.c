...
int jitter_packet_encode(const jitter_packet_t *pkt,
                           ...
                           size_t                 buf_sz) {
    if (!pkt || !buf) return -1;
    if (pkt->payload_len > JITTER_MAX_PAYLOAD) return -1;
...
int jitter_packet_decode(const uint8_t  *buf,
                           size_t          buf_sz,
                           jitter_packet_t *pkt) {
    if (!buf || !pkt || buf_sz < JITTER_PKT_HDR_SIZE) return -1;
    if (r32le(buf) != (uint32_t)JITTER_MAGIC) return -1;
    ...
    if (pkt->payload_len > JITTER_MAX_PAYLOAD) return -1;
    if (buf_sz < JITTER_PKT_HDR_SIZE + (size_t)pkt->payload_len) return -1;
    if (pkt->payload_len > 0)
