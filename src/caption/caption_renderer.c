...
static void blend(uint8_t *rgba, uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
    if (a == 0) return;
    if (a == 255) { rgba[0]=r; rgba[1]=g; rgba[2]=b; rgba[3]=255; return; }
    float fa = a / 255.0f;
    float fi = 1.0f - fa;
...
caption_renderer_t *caption_renderer_create(
    ...
    if (config) {
        ...
        r->font_scale = (config->font_scale >= 1 && config->font_scale <= 4)
                        ? config->font_scale : 2;
...
static void draw_glyph(uint8_t *pixels, int width, int height, int stride,
                        int px, int py, unsigned char ch,
                        uint8_t fr, uint8_t fgn, uint8_t fb, uint8_t fa,
                        int scale) {
    if (ch < 32 || ch > 127) ch = '?';
...
static void draw_caption(caption_renderer_t    *r,
                          uint8_t               *pixels,
                          int                    width,
                          int                    height,
                          int                    stride,
    ...
    for (int i = 0; i < (int)event->text_len; i++) {
        draw_glyph(pixels, width, height, stride,
                   gx + i * glyph_w, gy,
                   (unsigned char)event->text[i],
                   fg_r, fg_gn, fg_b, fg_a, scale);
    }
}
...
int caption_renderer_draw(caption_renderer_t    *r,
                           ...
                           int                    n,
                           uint64_t               now_us) {
    if (!r || !pixels || !events || width <= 0 || height <= 0) return 0;

    int rendered = 0;
    for (int i = 0; i < n; i++) {
        if (!caption_event_is_active(&events[i], now_us)) continue;
