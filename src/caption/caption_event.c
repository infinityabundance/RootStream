...
int caption_event_decode(const uint8_t   *buf,
    ...
    if (!buf || !event || buf_sz < CAPTION_HDR_SIZE) return -1;
    if (r32le(buf) != (uint32_t)CAPTION_MAGIC) return -1;
    ...
    if (event->text_len > CAPTION_MAX_TEXT_BYTES) return -1;
    if (buf_sz < CAPTION_HDR_SIZE + (size_t)event->text_len) return -1;
...
bool caption_event_is_active(const caption_event_t *event, uint64_t now_us) {
    ...
    return now_us >= event->pts_us &&
           now_us <  event->pts_us + event->duration_us;
}
