 ...
 */
int key_exchange_compute_shared_secret(const uint8_t *secret_key, const uint8_t *peer_public_key,
 ...
 */
int key_exchange_x3dh_create_bundle(const key_exchange_keypair_t *identity_keypair,
    ...
    /* Copy identity key */
    memcpy(bundle->identity_key, identity_keypair->public_key, KEY_EXCHANGE_PUBLIC_KEY_BYTES);
 ...
 * X3DH initiator
 */
int key_exchange_x3dh_initiator(const key_exchange_x3dh_bundle_t *recipient_bundle,
                                key_exchange_x3dh_init_t *init_msg, uint8_t *shared_secret) {
    ...
    if (key_exchange_compute_shared_secret(ephemeral.secret_key, recipient_bundle->signed_prekey,
                                           dh1) != 0) {
    ...
    if (key_exchange_compute_shared_secret(ephemeral.secret_key, recipient_bundle->identity_key,
                                           dh2) != 0) {
    ...
    memcpy(combined + KEY_EXCHANGE_SHARED_SECRET_BYTES, dh2, KEY_EXCHANGE_SHARED_SECRET_BYTES);

    const uint8_t *info = (const uint8_t *)"RootStreamX3DH";
    crypto_prim_hkdf(combined, sizeof(combined), NULL, 0, info, strlen((const char *)info),
 ...
 * X3DH responder
 */
int key_exchange_x3dh_responder(const key_exchange_x3dh_init_t *init_msg,
    ...
    uint8_t dh1[KEY_EXCHANGE_SHARED_SECRET_BYTES];
    if (key_exchange_compute_shared_secret(signed_prekey->secret_key, init_msg->ephemeral_key,
                                           dh1) != 0) {
        return -1;
    }
    ...
    uint8_t dh2[KEY_EXCHANGE_SHARED_SECRET_BYTES];
    if (key_exchange_compute_shared_secret(identity_keypair->secret_key, init_msg->ephemeral_key,
                                           dh2) != 0) {
        crypto_prim_secure_wipe(dh1, sizeof(dh1));
        return -1;
    }
...
int key_exchange_derive_session_keys(const uint8_t *shared_secret, uint8_t *client_to_server_key,
                                     uint8_t *server_to_client_key, uint8_t *client_nonce,
    ...
    if (client_to_server_key) {
        crypto_prim_hkdf(shared_secret, KEY_EXCHANGE_SHARED_SECRET_BYTES, NULL, 0, info_c2s,
                         strlen((const char *)info_c2s), client_to_server_key,
                         CRYPTO_PRIM_KEY_BYTES);
    }

    if (server_to_client_key) {
        crypto_prim_hkdf(shared_secret, KEY_EXCHANGE_SHARED_SECRET_BYTES, NULL, 0, info_s2c,
                         strlen((const char *)info_s2c), server_to_client_key,
                         CRYPTO_PRIM_KEY_BYTES);
    }

    if (client_nonce) {
        crypto_prim_hkdf(shared_secret, KEY_EXCHANGE_SHARED_SECRET_BYTES, NULL, 0, info_nc,
                         strlen((const char *)info_nc), client_nonce, CRYPTO_PRIM_NONCE_BYTES);
    }

    if (server_nonce) {
        crypto_prim_hkdf(shared_secret, KEY_EXCHANGE_SHARED_SECRET_BYTES, NULL, 0, info_ns,
                         strlen((const char *)info_ns), server_nonce, CRYPTO_PRIM_NONCE_BYTES);
    }
