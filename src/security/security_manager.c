...
static security_config_t g_config = {.enable_audit_logging = true,
...
int security_manager_init(const security_config_t *config) {
    ...
    audit_log_event(AUDIT_EVENT_SECURITY_ALERT, NULL, NULL, "Security manager initialized", false);
...
int security_manager_authenticate(const char *username, const char *password, char *session_token) {
    ...
    if (g_config.enable_rate_limiting &&
        ...
        audit_log_event(AUDIT_EVENT_LOGIN_FAILED, username, NULL, "Rate limited", false);
        return -1;
    }
    ...
    if (!user_auth_verify_password(password, stored_hash)) {
        ...
        audit_log_event(AUDIT_EVENT_LOGIN_FAILED, username, NULL, "Invalid password", false);
        return -1;
    }
    ...
    if (session_manager_create(username, session_token) != 0) {
        audit_log_event(AUDIT_EVENT_LOGIN_FAILED, username, NULL, "Session creation failed", false);
        return -1;
    }

    audit_log_event(AUDIT_EVENT_LOGIN, username, NULL, "Login successful", false);
    audit_log_event(AUDIT_EVENT_SESSION_CREATED, username, NULL, session_token, false);
...
int security_manager_logout(const char *session_token) {
    ...
    if (ret == 0) {
        audit_log_event(AUDIT_EVENT_LOGOUT, NULL, NULL, session_token, false);
    }
...
int security_manager_key_exchange(const uint8_t *peer_public_key, uint8_t *shared_secret) {
    ...
    int ret = key_exchange_compute_shared_secret(g_our_keypair.secret_key, peer_public_key,
                                                 shared_secret);

    if (ret == 0) {
        audit_log_event(AUDIT_EVENT_KEY_EXCHANGE, NULL, NULL, "Key exchange completed", false);
    } else {
        audit_log_event(AUDIT_EVENT_KEY_EXCHANGE, NULL, NULL, "Key exchange failed", true);
    }
...
int security_manager_encrypt(const uint8_t *plaintext, size_t plaintext_len, const uint8_t *key,
    ...
    return crypto_prim_chacha20poly1305_encrypt(plaintext, plaintext_len, key, nonce, NULL,
                                                0, /* No AAD */
                                                ciphertext, tag);
}
...
int security_manager_decrypt(const uint8_t *ciphertext, size_t ciphertext_len, const uint8_t *key,
    ...
    /* Decrypt */
    int ret = crypto_prim_chacha20poly1305_decrypt(ciphertext, ciphertext_len, key, nonce, NULL,
                                                   0, /* No AAD */
                                                   tag, plaintext);
...
int security_manager_get_stats(char *stats_json, size_t buf_len) {
    ...
    snprintf(stats_json, buf_len,
             ...
             "\"rate_limiting\":%s"
             "}",
             g_config.enable_audit_logging ? "true" : "false", g_config.session_timeout_sec,
...
void security_manager_cleanup(void) {
    ...
    audit_log_event(AUDIT_EVENT_SECURITY_ALERT, NULL, NULL, "Security manager shutdown", false);
