...
int discovery_broadcast_announce(rootstream_ctx_t *ctx) {
    ...
    if (get_local_ip(local_ip, sizeof(local_ip), broadcast_ip, sizeof(broadcast_ip)) < 0) {
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &broadcast_flag, sizeof(broadcast_flag)) < 0) {
    ...
    strncpy(pkt.rootstream_code, ctx->keypair.rootstream_code, sizeof(pkt.rootstream_code) - 1);
...
int discovery_broadcast_listen(rootstream_ctx_t *ctx, int timeout_ms) {
    ...
    ssize_t recv_len =
        recvfrom(sock, &pkt, sizeof(pkt), 0, (struct sockaddr *)&from_addr, &from_len);
    close(sock);
    ...
    printf("✓ Discovered peer: %s (%s:%u, code: %.16s..., %s)\n", pkt.hostname, peer_ip,
           ntohs(pkt.listen_port), pkt.rootstream_code,
    ...
    if (!already_exists && ctx->num_peers < MAX_PEERS) {
        ...
        strncpy(peer->rootstream_code, pkt.rootstream_code, sizeof(peer->rootstream_code) - 1);
