...
probe_estimator_t *probe_estimator_create(void) {
    ...
    if (pe) pe->owd_min_us = DBL_MAX;
    return pe;
}

void probe_estimator_destroy(probe_estimator_t *pe) { free(pe); }
...
int probe_estimator_observe(probe_estimator_t *pe,
                              ...
                              uint64_t            recv_ts_us,
                              uint32_t            size_bytes) {
    if (!pe || recv_ts_us < send_ts_us) return -1;
    ...
    if (owd < pe->owd_min_us) pe->owd_min_us = owd;
    if (owd > pe->owd_max_us) pe->owd_max_us = owd;
