...
sl_table_t *sl_table_create(int max_sessions) {
    if (max_sessions < 1 || max_sessions > SL_MAX_SLOTS) return NULL;
...
void sl_table_destroy(sl_table_t *t) { free(t); }

int sl_table_count(const sl_table_t *t) { return t ? t->count : 0; }
...
sl_entry_t *sl_table_add(sl_table_t *t,
                           ...
                           uint64_t    start_us) {
    if (!t) return NULL;
    if (t->count >= t->max_sessions) return NULL;  /* cap reached */
    for (int i = 0; i < SL_MAX_SLOTS; i++) {
...
void sl_table_foreach(sl_table_t *t,
    ...
    if (!t || !cb) return;
    for (int i = 0; i < SL_MAX_SLOTS; i++)
        if (t->entries[i].in_use) cb(&t->entries[i], user);
}
