...
void rm_table_destroy(rm_table_t *t) { free(t); }

int rm_table_count(const rm_table_t *t) { return t ? t->count : 0; }
...
rm_entry_t *rm_table_add(rm_table_t *t,
    ...
    if (!t || t->count >= RM_MAX_SLOTS) return NULL;
    for (int i = 0; i < RM_MAX_SLOTS; i++) {
        if (!t->entries[i].in_use) {
            if (rm_entry_init(&t->entries[i], request_id,
                              now_us, base_delay_us, max_attempts) != 0)
                return NULL;
            t->count++;
            return &t->entries[i];
        }
...
int rm_table_tick(rm_table_t *t,
    ...
    int processed = 0;
    for (int i = 0; i < RM_MAX_SLOTS; i++) {
        if (!t->entries[i].in_use) continue;
        if (!rm_entry_is_due(&t->entries[i], now_us)) continue;

        processed++;
        if (cb) cb(&t->entries[i], user);
