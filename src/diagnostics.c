...
void diagnostics_print_available_backends(rootstream_ctx_t *ctx) {
    ...
    bool xdotool_found =
        (access("/usr/bin/xdotool", X_OK) == 0 || access("/usr/local/bin/xdotool", X_OK) == 0 ||
...
void diagnostics_print_active_backends(rootstream_ctx_t *ctx) {
    ...
    printf("  Discovery:     %s\n", ctx->active_backend.discovery_name
                                        ...
                                        : "uninitialized");
    printf("  Input:         %s\n",
           ctx->active_backend.input_name ? ctx->active_backend.input_name : "uninitialized");
    printf("  GUI:           %s\n",
           ctx->active_backend.gui_name ? ctx->active_backend.gui_name : "uninitialized");
