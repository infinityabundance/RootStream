...
int discovery_save_peer_to_history(rootstream_ctx_t *ctx, const char *hostname, uint16_t port,
    ...
    if (rootstream_code) {
        strncpy(entry->rootstream_code, rootstream_code, sizeof(entry->rootstream_code) - 1);
...
void discovery_list_peer_history(rootstream_ctx_t *ctx) {
    ...
    printf("\nSaved Peers:\n");
    for (int i = 0; i < ctx->num_peer_history; i++) {
        printf("  %d. %s (%s)\n", i + 1, ctx->peer_history_entries[i].hostname,
