...
hs_stats_t *hs_stats_create(void) {
    ...
    if (st) st->min_rtt_us = DBL_MAX;
    return st;
}

void hs_stats_destroy(hs_stats_t *st) { free(st); }
...
int hs_stats_complete(hs_stats_t *st, uint64_t now_us) {
    ...
    if (st->pending_start_us > 0 && now_us >= st->pending_start_us) {
        ...
        st->rtt_sum_us += rtt;
        if (rtt < st->min_rtt_us) st->min_rtt_us = rtt;
        if (rtt > st->max_rtt_us) st->max_rtt_us = rtt;
    }
...
int hs_stats_snapshot(const hs_stats_t *st, hs_stats_snapshot_t *out) {
    ...
    out->avg_rtt_us = (st->successes > 0) ?
                      (st->rtt_sum_us / (double)st->successes) : 0.0;
