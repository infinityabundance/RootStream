...
int hs_message_encode(const hs_message_t *msg,
                        ...
                        size_t              buf_sz) {
    if (!msg || !buf) return -1;
    if (msg->payload_len > HS_MAX_PAYLOAD) return -1;
...
int hs_message_decode(const uint8_t *buf,
                        size_t         buf_sz,
                        hs_message_t  *msg) {
    if (!buf || !msg || buf_sz < (size_t)HS_MSG_HDR_SIZE) return -1;
    if (r32le(buf) != (uint32_t)HS_MSG_MAGIC) return -1;

    uint16_t plen = r16le(buf + 8);
    if (plen > HS_MAX_PAYLOAD) return -1;
    if (buf_sz < (size_t)(HS_MSG_HDR_SIZE + plen)) return -1;
    ...
    if (r32le(buf + 12) != expected) return -1;
...
const char *hs_msg_type_name(hs_msg_type_t t) {
    switch (t) {
    case HS_MSG_HELLO:    return "HELLO";
    case HS_MSG_HELLO_ACK:return "HELLO_ACK";
    case HS_MSG_AUTH:     return "AUTH";
    case HS_MSG_AUTH_ACK: return "AUTH_ACK";
    case HS_MSG_CONFIG:   return "CONFIG";
    case HS_MSG_READY:    return "READY";
    case HS_MSG_ERROR:    return "ERROR";
    case HS_MSG_BYE:      return "BYE";
