...
noise_gate_state_t *noise_gate_create(const noise_gate_config_t *config) {
    ...
    if (s->hold_samples < 1) s->hold_samples = 1;
    s->gate_open = false;
    return s;
}
...
static void noise_gate_process(float *samples, size_t frame_count,
...
spectral_sub_state_t *spectral_sub_create(
    ...
    s->floor_linear = dbfs_to_linear(config->floor_db < 0.0f
                                     ? config->floor_db : -60.0f);
...
static void spectral_sub_process(float *samples, size_t frame_count,
                                  int channels, void *user_data) {
    ...
    if (target_rms < s->floor_linear) target_rms = s->floor_linear;
