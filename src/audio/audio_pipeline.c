...
int audio_pipeline_remove_node(audio_pipeline_t *pipeline, const char *name) {
    ...
    for (size_t i = 0; i < pipeline->node_count; i++) {
        if (pipeline->nodes[i].name && strcmp(pipeline->nodes[i].name, name) == 0) {
            memmove(&pipeline->nodes[i], &pipeline->nodes[i + 1],
                    (pipeline->node_count - i - 1) * sizeof(audio_filter_node_t));
            pipeline->node_count--;
            return 0;
        }
    }
...
void audio_pipeline_process(audio_pipeline_t *pipeline, float *samples, size_t frame_count) {
    if (!pipeline || !samples || frame_count == 0)
        return;

    for (size_t i = 0; i < pipeline->node_count; i++) {
        audio_filter_node_t *n = &pipeline->nodes[i];
        if (n->enabled && n->process) {
            n->process(samples, frame_count, pipeline->channels, n->user_data);
        }
    }
}
