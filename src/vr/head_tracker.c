...
static XrVector3f quat_rotate_vector(const XrQuaternionf *q, const XrVector3f *v) {
    ...
    XrVector3f cross_uv = {u.y * v->z - u.z * v->y, u.z * v->x - u.x * v->z,
                           u.x * v->y - u.y * v->x};

    XrVector3f result = {2.0f * dot_uv * u.x + (s * s - dot_uu) * v->x + 2.0f * s * cross_uv.x,
...
HeadTrackingData head_tracker_predict_pose(HeadTracker *tracker, uint32_t prediction_ms) {
    ...
    float angularSpeed =
        sqrtf(tracker->currentPose.angularVelocity.x * tracker->currentPose.angularVelocity.x +
