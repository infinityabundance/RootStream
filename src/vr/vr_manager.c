...
int vr_manager_init(VRManager *manager, const VRConfig *config) {
    ...
    if (stereoscopic_renderer_init(manager->stereoRenderer, manager->config.renderWidth,
...
int vr_manager_render_frame(VRManager *manager, const VideoFrame *frame) {
    ...
    // Render left eye
    if (stereoscopic_renderer_render_left_eye(manager->stereoRenderer, frame, leftProj, leftView) <
        0) {
    ...
    // Render right eye
    if (stereoscopic_renderer_render_right_eye(manager->stereoRenderer, frame, rightProj,
                                               rightView) < 0) {
...
int vr_manager_get_projection_matrices(VRManager *manager, float leftProj[16],
                                       float rightProj[16]) {
