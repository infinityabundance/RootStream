...
void vr_latency_optimizer_record_frame_end(VRLatencyOptimizer *optimizer) {
    if (!optimizer || !optimizer->frame_started)
        return;
    ...
    if (optimizer->frame_count < FRAME_WINDOW)
        optimizer->frame_count++;
    optimizer->frame_started = false;
}
...
VRLatencyMetrics vr_latency_optimizer_get_metrics(VRLatencyOptimizer *optimizer) {
    VRLatencyMetrics m;
    memset(&m, 0, sizeof(m));
    if (!optimizer || optimizer->frame_count == 0)
        return m;
...
bool vr_latency_optimizer_is_reprojection_needed(VRLatencyOptimizer *optimizer) {
    if (!optimizer || optimizer->mode == VR_REPROJ_NONE)
        return false;
    if (optimizer->frame_count == 0)
        return false;
