...
static bool ray_plane_intersect(const XrVector3f *rayOrigin, const XrVector3f *rayDir,
    ...
    XrVector3f diff = {planePos->x - rayOrigin->x, planePos->y - rayOrigin->y,
    ...
    XrVector3f localHit = {hitPoint->x - planePos->x, hitPoint->y - planePos->y,
                           hitPoint->z - planePos->z};

    return (fabsf(localHit.x) <= planeWidth / 2.0f && fabsf(localHit.y) <= planeHeight / 2.0f);
}
...
bool vr_ui_raycast(VRUIFramework *framework, const XrVector3f *rayOrigin,
    ...
    for (uint32_t i = 0; i < MAX_UI_PANELS; i++) {
        ...
        if (ray_plane_intersect(rayOrigin, rayDirection, &framework->panels[i].position, &normal,
                                framework->panels[i].width, framework->panels[i].height,
                                &tempHit)) {
            float dist = sqrtf((tempHit.x - rayOrigin->x) * (tempHit.x - rayOrigin->x) +
