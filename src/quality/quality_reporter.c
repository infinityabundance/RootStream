...
int quality_report_json(const quality_stats_t *stats, uint64_t scene_changes, char *buf,
    ...
    int n =
        snprintf(buf, buf_sz,
                 ...
                 "\"scene_changes\":%llu"
                 "}",
                 (unsigned long long)stats->frames_total, (unsigned long long)stats->alerts_total,
                 stats->degraded ? "true" : "false", stats->avg_psnr, stats->min_psnr,
                 stats->avg_ssim, stats->min_ssim, (unsigned long long)scene_changes);
