...
quality_monitor_t *quality_monitor_create(
        const quality_monitor_config_t *config) {
    ...
    if (config) {
        ...
        m->window_size    = (config->window_size > 0 &&
                             config->window_size <= QUALITY_WINDOW_SIZE)
                            ? config->window_size : QUALITY_WINDOW_SIZE;
    } else {
...
void quality_monitor_push(quality_monitor_t *m, double psnr, double ssim) {
    ...
    if (m->filled < m->window_size) m->filled++;
    m->frames_total++;
    ...
    m->degraded = (avg_psnr < m->psnr_threshold ||
                   avg_ssim < m->ssim_threshold);
...
void quality_monitor_get_stats(const quality_monitor_t *m,
    ...
    for (int i = 0; i < n; i++) {
        ...
        sum_psnr += p;
        sum_ssim += s;
        if (p < min_psnr) min_psnr = p;
        if (s < min_ssim) min_ssim = s;
    }
