...
int fr_stats_record_frame(fr_stats_t *st, uint64_t interval_us) {
    ...
    if (interval_us < st->min_interval_us)
        st->min_interval_us = interval_us;
    if (interval_us > st->max_interval_us)
        st->max_interval_us = interval_us;
    return 0;
}
...
int fr_stats_snapshot(const fr_stats_t *st, fr_stats_snapshot_t *out) {
    ...
    out->avg_interval_us =
        (st->frame_count > 0) ? st->sum_interval_us / (double)st->frame_count : 0.0;
