...
const int cfb_chain_quality_len = (int)(sizeof(cfb_chain_quality) /
                                        sizeof(cfb_chain_quality[0]));
...
const int cfb_chain_compat_len = (int)(sizeof(cfb_chain_compat) /
                                       sizeof(cfb_chain_compat[0]));
...
const int cfb_chain_modern_len = (int)(sizeof(cfb_chain_modern) /
                                       sizeof(cfb_chain_modern[0]));
...
const int cfb_chain_discord_len = (int)(sizeof(cfb_chain_discord) /
                                        sizeof(cfb_chain_discord[0]));

/* ── Internal helpers ─────────────────────────────────────────────── */

/* Scan one pass through the chain.
 * require_hw: if true, only accept entries with hw_preferred=true. */
static int scan_chain(const creg_registry_t *r,
                      const uint8_t *chain, int chain_len,
                      bool require_hw, uint8_t options)
{
    ...
    for (int i = 0; i < chain_len; i++) {
        ...
        if (!creg_encode_available(r, cid)) continue;

        if (require_hw && !creg_hw_preferred(r, cid)) continue;
                         ...
                         cfb_result_t          *result)
{
    ...
    if (!r || !chain || chain_len <= 0) return CREG_VCODEC_H264;
                                ...
                                cfb_result_t          *result)
{
    ...
    if (preferred == CREG_VCODEC_LIBDAVE) {
        return cfb_select_best(r, preferred,
                               cfb_chain_discord, cfb_chain_discord_len,
                               options, result);
    }
    if (preferred >= CREG_VCODEC_VVC) {
        return cfb_select_best(r, preferred,
                               cfb_chain_quality, cfb_chain_quality_len,
                               options, result);
    }
    /* Default: modern balanced chain */
    return cfb_select_best(r, preferred,
                           cfb_chain_modern, cfb_chain_modern_len,
                           options, result);
}
