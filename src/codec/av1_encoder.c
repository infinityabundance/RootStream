    ...
    /* Try to open DRM render node and query AV1 encode capability */
    {
        ...
        if (drm_fd >= 0) {
            ...
            if (vaInitialize(dpy, &major, &minor) == VA_STATUS_SUCCESS) {
                ...
                VAEntrypoint eps[10];
                int n_eps = 0;
                VAStatus s = vaQueryConfigEntrypoints(
                    dpy, VAProfileAV1Profile0, eps, &n_eps);
                bool found = false;
                if (s == VA_STATUS_SUCCESS) {
                    for (int i = 0; i < n_eps; i++) {
                        if (eps[i] == VAEntrypointEncSlice) { found = true; break; }
                    }
                }
                vaTerminate(dpy);
                close(drm_fd);
                if (found) return AV1_BACKEND_VAAPI;
            } else {
                close(drm_fd);
            }
        }
...
av1_encoder_ctx_t *av1_encoder_create(const av1_encoder_config_t *config) {
    ...
    if (backend == AV1_BACKEND_NONE) {
        ...
        if (backend == AV1_BACKEND_NONE) {
            fprintf(stderr, "av1_encoder: no AV1 backend available — "
    ...
    case AV1_BACKEND_LIBAOM: {
        ...
        aom_cfg.rc_target_bitrate = config->bitrate_kbps > 0
        ...
        if (!ctx->aom_ctx) { free(ctx); return NULL; }

        aom_codec_err_t err = aom_codec_enc_init(
            ctx->aom_ctx, iface, &aom_cfg, 0);
        if (err != AOM_CODEC_OK) {
        ...
        aom_img_alloc(&ctx->aom_img, AOM_IMG_FMT_I420,
                      (unsigned)config->width, (unsigned)config->height, 1);
        break;
    }
#endif /* HAVE_LIBAOM */
    ...
    case AV1_BACKEND_SVT: {
        ...
        if (svt_av1_enc_init_handle(&ctx->svt_handle, NULL, &svt_cfg)
                != EB_ErrorNone) {
        ...
        svt_cfg.target_bit_rate = config->bitrate_kbps > 0
                                  ? config->bitrate_kbps * 1000 : 4000000;
    ...
    fprintf(stderr, "av1_encoder: initialised backend=%d (%s) %dx%d @ %dfps\n",
            ...
            backend == AV1_BACKEND_NVENC  ? "nvenc" : "unknown",
...
int av1_encoder_encode(av1_encoder_ctx_t *ctx,
                       ...
                       bool              *is_keyframe)
{
    if (!ctx || !yuv420 || !out || !out_size) return -1;
    ...
    if (yuv_size < expected) return -1;  /* truncated input frame */
    ...
    case AV1_BACKEND_LIBAOM: {
        ...
        memcpy(ctx->aom_img.planes[0], yuv420,
               (size_t)(width * height));
        memcpy(ctx->aom_img.planes[1], yuv420 + width * height,
               (size_t)(width * height / 4));
        memcpy(ctx->aom_img.planes[2], yuv420 + width * height * 5 / 4,
               (size_t)(width * height / 4));

        aom_enc_frame_flags_t flags = kf ? AOM_EFLAG_FORCE_KF : 0;
        aom_codec_err_t err = aom_codec_encode(
            ctx->aom_ctx, &ctx->aom_img,
            (aom_codec_pts_t)ctx->frames_encoded,
            1 /* duration */, flags);
        if (err != AOM_CODEC_OK) return -1;
        ...
        while ((pkt = aom_codec_get_cx_data(ctx->aom_ctx, &iter)) != NULL) {
            if (pkt->kind == AOM_CODEC_CX_FRAME_PKT) {
                if (written + pkt->data.frame.sz > *out_size) return -1;
                memcpy(out + written, pkt->data.frame.buf,
                       pkt->data.frame.sz);
                written += pkt->data.frame.sz;
                if (pkt->data.frame.flags & AOM_FRAME_IS_KEY) kf = true;
            }
        }
        *out_size = written;
        if (is_keyframe) *is_keyframe = kf;
        ctx->frames_encoded++;
    ...
    case AV1_BACKEND_SVT: {
        ...
        ctx->svt_in->flags         = 0;
        ctx->svt_in->pic_type      = kf ? EB_AV1_KEY_PICTURE
        ...
        if (svt_av1_enc_send_picture(ctx->svt_handle, ctx->svt_in)
                != EB_ErrorNone) return -1;

        /* Retrieve compressed output */
        EbBufferHeaderType *out_buf = NULL;
        if (svt_av1_enc_get_packet(ctx->svt_handle, &out_buf, 0)
                != EB_ErrorNone) return -1;

        if (!out_buf || out_buf->n_filled_len > *out_size) {
            if (out_buf) svt_av1_enc_release_out_buffer(&out_buf);
            return -1;
        }
        ...
        *out_size = out_buf->n_filled_len;
        ...
        if (is_keyframe) *is_keyframe = is_kf;
        ctx->frames_encoded++;
                           ...
                           float    *fps_actual)
{
    ...
    if (bitrate_kbps && ctx->frames_encoded > 0)
        *bitrate_kbps = (uint32_t)(ctx->bytes_out_total * 8
                                   / ctx->frames_encoded
                                   / 1000
                                   * ctx->config.fps);
    if (fps_actual) *fps_actual = (float)ctx->config.fps;
}
