...
void reassemble_ctx_destroy(reassemble_ctx_t *c) { free(c); }

int reassemble_count(const reassemble_ctx_t *c) {
    if (!c) return 0;
    int n = 0;
    for (int i = 0; i < REASSEMBLE_SLOTS; i++)
        if (c->slots[i].in_use) n++;
    return n;
}

static reassemble_slot_t *find_slot(reassemble_ctx_t *c,
                                     uint32_t stream_id,
                                     uint32_t frame_seq) {
    for (int i = 0; i < REASSEMBLE_SLOTS; i++)
        if (c->slots[i].in_use &&
...
reassemble_slot_t *reassemble_receive(reassemble_ctx_t *c,
    ...
    if (!c || !h) return NULL;
    if (h->chunk_count == 0 ||
        h->chunk_count > REASSEMBLE_MAX_CHUNKS ||
        h->chunk_idx   >= h->chunk_count) return NULL;
    ...
    uint32_t full_mask = (h->chunk_count == 32)
                         ? 0xFFFFFFFFu
                         : (1u << h->chunk_count) - 1u;
