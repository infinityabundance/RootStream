...
int cs_stats_record(cs_stats_t *st, int64_t offset_us, int64_t rtt_us) {
    ...
    if (offset_us < st->min_offset_us)
        st->min_offset_us = offset_us;
    if (offset_us > st->max_offset_us)
        st->max_offset_us = offset_us;
    if (rtt_us < st->min_rtt_us)
        st->min_rtt_us = rtt_us;
    if (rtt_us > st->max_rtt_us)
        st->max_rtt_us = rtt_us;
    return 0;
}
...
int cs_stats_snapshot(const cs_stats_t *st, cs_stats_snapshot_t *out) {
    ...
    out->avg_offset_us =
        (st->sample_count > 0) ? st->sum_offset_us / (double)st->sample_count : 0.0;
    out->min_rtt_us = (st->sample_count > 0) ? st->min_rtt_us : 0;
    out->max_rtt_us = (st->sample_count > 0) ? st->max_rtt_us : 0;
    out->avg_rtt_us = (st->sample_count > 0) ? st->sum_rtt_us / (double)st->sample_count : 0.0;
