...
static uint16_t pick_height(uint16_t max_height, uint32_t bps, uint32_t max_bps) {
    ...
    for (int i = 0; i < n_std_heights; i++) {
        ...
        if (std_heights[i] <= target) {
            best = std_heights[i];
            break;
        }
    }
    if (best == 0)
        best = std_heights[n_std_heights - 1];
    return best;
}
...
int ladder_build(const ladder_params_t *p, ladder_rung_t *rungs, int *n_out) {
    if (!p || !rungs || !n_out)
        return -1;
    if (p->max_bps == 0 || p->min_bps == 0 || p->max_bps < p->min_bps)
        return -1;
    if (p->step_ratio <= 0.0f || p->step_ratio >= 1.0f)
        return -1;
    if (p->max_height == 0 || p->max_fps <= 0.0f)
        return -1;
    ...
    while (bps >= p->min_bps && n < LADDER_MAX_RUNGS) {
        ...
        if (next_bps >= bps)
            break; /* safety: ensure strictly decreasing */
        bps = next_bps;
    }
