...
int load_balancer_add_stream(load_balancer_t *balancer, uint32_t stream_id,
...
int load_balancer_remove_stream(load_balancer_t *balancer, uint32_t stream_id) {
    ...
    for (uint32_t i = 0; i < MAX_STREAMS; i++) {
        if (balancer->streams[i].active && balancer->streams[i].stream_id == stream_id) {
...
int load_balancer_allocate_bandwidth(load_balancer_t *balancer, uint32_t total_bandwidth_mbps) {
    if (!balancer) {
        return -1;
    }
...
uint32_t load_balancer_get_stream_bitrate(load_balancer_t *balancer, uint32_t stream_id) {
    if (!balancer) {
        return 0;
    }
...
int load_balancer_allocate_fair_share(load_balancer_t *balancer) {
    ...
    return load_balancer_allocate_bandwidth(balancer, balancer->total_available_bandwidth_mbps);
}
