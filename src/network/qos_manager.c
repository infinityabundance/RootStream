...
int qos_manager_register_traffic_class(qos_manager_t *manager, const char *name,
                                       packet_priority_t priority, uint32_t max_rate_kbps) {
...
packet_priority_t qos_manager_classify_packet(qos_manager_t *manager, const uint8_t *packet_data,
                                              size_t packet_len) {
...
bool qos_manager_should_drop_packet(qos_manager_t *manager, packet_priority_t priority,
                                    size_t queue_depth) {
    if (!manager) {
        return false;
    }
...
uint32_t qos_manager_get_packets_dropped(qos_manager_t *manager, packet_priority_t priority) {
    if (!manager) {
        return 0;
    }
...
uint32_t qos_manager_get_queue_depth(qos_manager_t *manager, packet_priority_t priority) {
    if (!manager) {
        return 0;
    }
