...
int abr_controller_add_profile(abr_controller_t *controller, uint32_t bitrate_kbps, uint32_t width,
                               uint32_t height, uint32_t fps, const char *codec,
                               const char *preset) {
...
const bitrate_profile_t *abr_controller_get_recommended_profile(abr_controller_t *controller) {
    ...
    if (controller->current_profile_index < controller->profile_count - 1) {
        const bitrate_profile_t *next =
            &controller->profiles[controller->current_profile_index + 1];
...
int abr_controller_set_target_bitrate(abr_controller_t *controller, uint32_t bitrate_kbps) {
    if (!controller || controller->profile_count == 0) {
        return -1;
    }
