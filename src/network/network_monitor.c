...
int network_monitor_record_packet_sent(network_monitor_t *monitor, uint32_t sequence,
...
int network_monitor_record_packet_ack(network_monitor_t *monitor, uint32_t sequence,
                                      uint64_t timestamp_us) {
    if (!monitor) {
        return -1;
    }
    ...
    for (uint32_t i = 0; i < monitor->pending_count; i++) {
        if (monitor->pending_packets[i].sequence == sequence) {
            ...
            if (monitor->rtt_samples == 0) {
                ...
                monitor->rtt_ewma =
                    (1.0f - RTT_SMOOTH_FACTOR) * monitor->rtt_ewma + RTT_SMOOTH_FACTOR * rtt_ms;
                monitor->rtt_var_ewma =
                    (1.0f - RTT_SMOOTH_FACTOR) * monitor->rtt_var_ewma + RTT_SMOOTH_FACTOR * delta;
            }
...
int network_monitor_record_packet_lost(network_monitor_t *monitor, uint32_t sequence) {
    if (!monitor) {
        return -1;
    }
...
int network_monitor_update_bandwidth_estimate(network_monitor_t *monitor, uint32_t delivered_bytes,
    ...
    if (monitor->bw_estimate_time_us == 0) {
        ...
        monitor->estimated_bw_mbps = (uint32_t)(0.8f * monitor->estimated_bw_mbps + 0.2f * mbps);
    }
...
int network_monitor_estimate_bandwidth_aimd(network_monitor_t *monitor, bool congestion_detected) {
    if (!monitor) {
        return -1;
    }
