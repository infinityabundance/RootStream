...
network_optimizer_t *network_optimizer_create(void) {
    ...
    if (!optimizer->monitor || !optimizer->abr || !optimizer->qos || !optimizer->bandwidth_est ||
        !optimizer->socket_tuning) {
...
int network_optimizer_add_profile(network_optimizer_t *optimizer, uint32_t bitrate_kbps,
                                  uint32_t width, uint32_t height, uint32_t fps, const char *codec,
                                  const char *preset) {
    if (!optimizer || !optimizer->abr) {
        return -1;
    }

    return abr_controller_add_profile(optimizer->abr, bitrate_kbps, width, height, fps, codec,
                                      preset);
}
...
int network_optimizer_optimize(network_optimizer_t *optimizer) {
    ...
    bool congested = bandwidth_estimator_detect_congestion(
        optimizer->bandwidth_est, conditions.rtt_ms, conditions.packet_loss_percent);
    ...
    if (profile && profile->bitrate_kbps != optimizer->last_bitrate_kbps) {
        ...
        if (optimizer->callbacks.on_bitrate_changed) {
            optimizer->callbacks.on_bitrate_changed(optimizer->callbacks.user_data,
...
int network_optimizer_record_packet_sent(network_optimizer_t *optimizer, uint32_t sequence,
...
int network_optimizer_record_packet_ack(network_optimizer_t *optimizer, uint32_t sequence,
                                        uint64_t timestamp_us) {
    if (!optimizer || !optimizer->monitor) {
        return -1;
    }
...
int network_optimizer_record_packet_lost(network_optimizer_t *optimizer, uint32_t sequence) {
    if (!optimizer || !optimizer->monitor) {
        return -1;
    }
...
int network_optimizer_tune_socket(network_optimizer_t *optimizer, int socket, bool low_latency) {
...
char *network_optimizer_get_diagnostics_json(network_optimizer_t *optimizer) {
    ...
    uint32_t bw_estimate =
        bandwidth_estimator_get_estimated_bandwidth_mbps(optimizer->bandwidth_est);
    ...
    snprintf(json, 1024,
             ...
             "  }\n"
             "}",
             conditions.rtt_ms, conditions.rtt_variance_ms, conditions.packet_loss_percent,
             conditions.bandwidth_mbps, conditions.congestion_level, bitrate, bw_estimate,
