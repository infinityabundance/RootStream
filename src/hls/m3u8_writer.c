...
int m3u8_write_live(const hls_segment_t *segments,
                     ...
                     char                *buf,
                     size_t               buf_sz) {
    if (!segments || !buf || buf_sz == 0) return -1;
    if (n <= 0 || window_size <= 0) return -1;
...
int m3u8_write_vod(const hls_segment_t *segments,
                    ...
                    char                *buf,
                    size_t               buf_sz) {
    if (!segments || !buf || buf_sz == 0 || n <= 0) return -1;
...
int m3u8_write_master(const char **uris,
                       ...
                       char        *buf,
                       size_t       buf_sz) {
    if (!uris || !bandwidths || !buf || buf_sz == 0 || n <= 0) return -1;
    ...
    for (int i = 0; i < n; i++) {
        if (width > 0 && height > 0) {
            r = snprintf(buf + pos, buf_sz - pos,
                         ...
                         bandwidths[i], width, height, uris[i]);
        } else {
            r = snprintf(buf + pos, buf_sz - pos,
