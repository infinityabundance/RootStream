...
int watermark_payload_decode(const uint8_t      *buf,
    ...
    size_t needed = WATERMARK_HDR_SIZE + WATERMARK_MAX_DATA_BYTES;
    if (!buf || !payload || buf_sz < needed) return -1;
    if (r32le(buf) != (uint32_t)WATERMARK_MAGIC) return -1;
...
int watermark_payload_to_bits(const watermark_payload_t *payload,
                               uint8_t                   *bits,
                               size_t                     max_bits) {
    if (!payload || !bits || max_bits < 64) return -1;
    for (int i = 0; i < 64; i++)
        bits[i] = (uint8_t)((payload->viewer_id >> (63 - i)) & 1);
    return 64;
}
...
int watermark_payload_from_bits(const uint8_t       *bits,
                                 int                  n_bits,
                                 watermark_payload_t  *payload) {
    if (!bits || !payload || n_bits != 64) return -1;
    uint64_t id = 0;
    for (int i = 0; i < 64; i++) {
        id <<= 1;
        id |= (bits[i] & 1);
