...
void event_ring_destroy(event_ring_t *r) { free(r); }

int event_ring_count(const event_ring_t *r) { return r ? r->count : 0; }

bool event_ring_is_empty(const event_ring_t *r) { return !r || r->count == 0; }

void event_ring_clear(event_ring_t *r) {
    if (r) { r->head = 0; r->count = 0; }
}
...
int event_ring_push(event_ring_t *r, const event_entry_t *e) {
    ...
    if (r->count < EVENT_RING_CAPACITY) r->count++;
    return 0;
}

int event_ring_get(const event_ring_t *r, int age, event_entry_t *out) {
    if (!r || !out || age < 0 || age >= r->count) return -1;
...
int event_ring_find_level(const event_ring_t *r,
                            ...
                            int                *out_ages,
                            int                 max_results) {
    if (!r || !out_ages || max_results <= 0) return 0;
    int found = 0;
