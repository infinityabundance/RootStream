...
int event_entry_decode(const uint8_t *buf, size_t buf_sz, event_entry_t *e) {
    if (!buf || !e || buf_sz < EVENT_ENTRY_HDR_SIZE) return -1;

    uint16_t msglen = r16le(buf + 12);
    if (msglen == 0 || msglen > EVENT_MSG_MAX) return -1;
    if (buf_sz < (size_t)(EVENT_ENTRY_HDR_SIZE + msglen)) return -1;
...
const char *event_level_name(event_level_t l) {
    switch (l) {
    case EVENT_LEVEL_DEBUG: return "DEBUG";
    case EVENT_LEVEL_INFO:  return "INFO";
    case EVENT_LEVEL_WARN:  return "WARN";
