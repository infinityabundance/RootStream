...
uint64_t scheduler_add(scheduler_t *sched, schedule_entry_t *entry) {
    ...
    for (int i = 0; i < SCHEDULER_MAX_ENTRIES; i++) {
        if (!sched->slots[i].used) { slot = i; break; }
    }
    if (slot < 0) {
...
int scheduler_tick(scheduler_t *sched, uint64_t now_us) {
    ...
    for (int i = 0; i < SCHEDULER_MAX_ENTRIES; i++) {
        ...
        if (!slot->used || slot->fired) continue;
        if (!(slot->entry.flags & SCHED_FLAG_ENABLED)) continue;
        if (slot->entry.start_us > now_us) continue;
        ...
        if (sched->fire_fn) {
            ...
            if (!sched->slots[i].used) continue;
        }
...
size_t scheduler_count(const scheduler_t *sched) {
    ...
    for (int i = 0; i < SCHEDULER_MAX_ENTRIES; i++) {
        if (sched->slots[i].used && !sched->slots[i].fired) count++;
    }
