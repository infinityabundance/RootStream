...
int schedule_store_save(const char             *path,
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (esz < 0) { fclose(f); remove(tmp); return -1; }

        uint8_t len[2];
        w16le(len, (uint16_t)esz);
        if (fwrite(len, 1, 2, f) != 2 ||
            fwrite(buf, 1, (size_t)esz, f) != (size_t)esz) {
            fclose(f); remove(tmp); return -1;
        }
    }
...
int schedule_store_load(const char       *path,
                         ...
                         size_t            max,
                         size_t           *out_count) {
    if (!path || !entries || !out_count) return -1;
    ...
    if (r32le(hdr) != (uint32_t)SCHEDULE_STORE_MAGIC) { fclose(f); return -1; }
    if (r16le(hdr + 4) != SCHEDULE_STORE_VERSION) { fclose(f); return -1; }
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (esz > SCHEDULE_ENTRY_MAX_SZ) break;

        uint8_t buf[SCHEDULE_ENTRY_MAX_SZ];
        if (fread(buf, 1, esz, f) != esz) break;
        if (schedule_entry_decode(buf, esz, &entries[loaded]) == 0) loaded++;
    }
