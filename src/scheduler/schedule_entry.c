...
int schedule_entry_decode(const uint8_t *buf, size_t buf_sz, schedule_entry_t *entry) {
    if (!buf || !entry || buf_sz < SCHEDULE_HDR_SIZE)
        return -1;
    if (r32le(buf) != (uint32_t)SCHEDULE_MAGIC)
        return -1;
    ...
    if (entry->title_len > SCHEDULE_MAX_TITLE)
        return -1;
    if (buf_sz < SCHEDULE_HDR_SIZE + (size_t)entry->title_len)
        return -1;
    if (entry->title_len > 0)
