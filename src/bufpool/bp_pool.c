...
bp_pool_t *bp_pool_create(int n_blocks, size_t block_size) {
    if (n_blocks < 1 || n_blocks > BP_MAX_BLOCKS || block_size == 0) return NULL;
    bp_pool_t *p = calloc(1, sizeof(*p));
    if (!p) return NULL;
    p->backing = calloc((size_t)n_blocks, block_size);
    if (!p->backing) { free(p); return NULL; }
    p->n_blocks   = n_blocks;
...
int bp_pool_release(bp_pool_t *p, bp_block_t *b) {
    ...
    if (b < p->blocks || b >= p->blocks + p->n_blocks) return -1;
...
int bp_pool_in_use(const bp_pool_t *p) { return p ? p->in_use : 0; }
int bp_pool_peak(const bp_pool_t *p)   { return p ? p->peak   : 0; }
int bp_pool_capacity(const bp_pool_t *p) { return p ? p->n_blocks : 0; }
